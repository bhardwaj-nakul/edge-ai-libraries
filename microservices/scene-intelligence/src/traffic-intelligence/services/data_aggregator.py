...
class DataAggregatorService:
    ...
    def __init__(self, config_service, weather_service, vlm_service):
        """
        Initialize data aggregator service.
        
        Args:
            config_service: Configuration service
            weather_service: Weather service for data enrichment
        ...
        self.vlm_analyzed_camera_data: Dict[str, CameraDataMessage] = {}  # direction -> VLM-analyzed data
        ...
        self.vlm_analyzed_traffic_snapshot: Optional[TrafficSnapshot] = None
    ...
    async def process_camera_image(self, camera_image: CameraImage) -> None:
        ...
        try:
            ...
            # print(f"\\n=== DATA AGGREGATOR IMAGE PROCESSING ===")
            # print(f"Processing camera image for direction: {direction}")
            # print(f"Camera ID: {camera_image.camera_id}")
            # print(f"Image Size: {camera_image.image_size_bytes} bytes")
            # print(f"Image Timestamp: {camera_image.timestamp}")
            # print(f"Current camera images: {list(self.current_camera_images.keys())}")
            # print("=" * 40)
    ...
    async def process_camera_data(self, 
                                camera_message: CameraDataMessage,
                                camera_image: Optional[CameraImage] = None) -> None:
        """
        Process incoming camera data and update current state.
        
        Args:
            camera_message: Camera data message from MQTT
            camera_image: Optional camera image data
        """
        try:
            direction = camera_message.direction
            
            # # print detailed camera data for debugging
            # print(f"\n=== DATA AGGREGATOR PROCESSING ===")
            # print(f"Processing camera data for direction: {direction}")
            # print(f"Camera ID: {camera_message.camera_id}")
            # print(f"Intersection ID: {camera_message.intersection_id}")
            ...
            # Update temporary camera image if provided
            if camera_image:
                self.temp_camera_images[direction] = camera_image
                logger.debug("Updated temporary camera image", 
                           direction=direction,
                           camera_id=camera_image.camera_id,
                           image_size=camera_image.image_size_bytes)
    ...
    def _create_temp_traffic_snapshot(self) -> Optional[TrafficSnapshot]:
        ...
        return TrafficSnapshot(
            ...
            weather_conditions=self.current_weather_data,
            camera_images=self.temp_camera_images.copy()
        )
    
    def _save_vlm_analyzed_data(self, traffic_snapshot: TrafficSnapshot) -> None:
        """Save data that was used in VLM analysis as the current analyzed data."""
        # Save the traffic snapshot that was analyzed
        self.vlm_analyzed_traffic_snapshot = traffic_snapshot
        
        # Copy temporary camera data to VLM-analyzed storage
        self.vlm_analyzed_camera_data = self.temp_camera_data.copy()
        self.vlm_analyzed_camera_images = self.temp_camera_images.copy()
        self.vlm_analyzed_intersection_data = self.temp_intersection_data
        ...
        logger.info("VLM-analyzed data saved",
                   ...
                   analyzed_cameras=list(self.vlm_analyzed_camera_data.keys()),
    ...
    async def _check_analysis_trigger(self) -> None:
        ...
        if self.last_analysis_time:
            time_since_analysis = (now - self.last_analysis_time).total_seconds()
            if time_since_analysis < self.vlm_trigger_duration:
                return
    ...
    async def _trigger_vlm_analysis(self) -> None:
        ...
        try:
            ...
            camera_images_list = list(self.temp_camera_images.values())
            logger.info("Triggering VLM analysis",
                       available_camera_images=len(camera_images_list))
            
            try:
                vlm_analysis = await self.vlm_service.analyze_traffic_non_blocking(
                    traffic_snapshot=traffic_snapshot,
                    camera_images=camera_images_list,
                    weather_data=self.current_weather_data
                )
                
                if vlm_analysis:
                    # VLM analysis successful - save the data that was analyzed
                    self._save_vlm_analyzed_data(traffic_snapshot)
    ...
    async def _check_sustained_traffic_analysis(self) -> None:
        ...
        if high_traffic_count >= 3:  # 3 out of 5 recent analyzed snapshots show high traffic
            ...
            if not self.last_analysis_time or \
               (datetime.utcnow() - self.last_analysis_time).total_seconds() > self.vlm_trigger_duration * 2:
        ...
        try:
            ...
            for direction, camera_image in self.vlm_analyzed_camera_images.items():
                camera_images_dict[f"{direction}_camera"] = {
                    'camera_id': camera_image.camera_id,
                    'direction': camera_image.direction,
                    'timestamp': camera_image.timestamp.isoformat(),
            ...
            response = TrafficIntelligenceResponse(
                ...
                weather_data=self.current_weather_data or self._get_default_weather(),
    ...
    def get_service_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "analyzed_camera_directions": list(self.vlm_analyzed_camera_data.keys()),
            "has_weather_data": self.current_weather_data is not None,
            "has_vlm_analysis": self.current_vlm_analysis is not None,
